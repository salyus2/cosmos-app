...
exports.getAllFactions = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error obteniendo facciones', error: err.message });
  }
};
...
exports.createFaction = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Solo admin puede crear facciones' });
    }
    ...
    res.status(500).json({ message: 'Error creando facción', error: err.message });
  }
};
...
exports.assignFaction = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Solo admin puede asignar facciones' });
    }
    ...
    res.status(500).json({ message: 'Error asignando facción', error: err.message });
  }
};